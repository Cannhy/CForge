...
LOCAL_EASY_PATH = os.path.join(
    os.path.dirname(__file__), "..", "..",
    "benchC_data", "data", "easy", "data.jsonl",
)
...
def _load_from_hf():
    ds = load_dataset(
        "cannhy/benchC", "easy", split="test",
        token=os.getenv("HF_TOKEN"), trust_remote_code=True,
    )
    return [EasyProblem(**p) for p in ds]
...
def load_code_generation_dataset(
    ...
    try:
        ...
        dataset = [EasyProblem(**p) for p in _load_local_jsonl(LOCAL_EASY_PATH)]
