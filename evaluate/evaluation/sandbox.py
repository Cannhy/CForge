...
def _run_c_function_execution(lib_path, func_name, signature: Dict, case, result_queue):
    # 抑制子进程中 C 代码 segfault / glibc 内存错误的打印输出
    # 必须在 fd 层面重定向，否则 glibc 的 C 层输出无法被 Python sys.stderr 拦截
    import sys
    faulthandler.disable()
    devnull_fd = os.open(os.devnull, os.O_WRONLY)
    os.dup2(devnull_fd, 2)  # 将 fd 2 (stderr) 重定向到 /dev/null
    os.close(devnull_fd)
...
def universal_c_tester(
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        process = _spawn_ctx.Process(target=_run_c_function_execution, args=(lib_file, func_name, signature, case, result_queue))
        process.start()
