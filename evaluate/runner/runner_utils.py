from evaluate.llm_styles import LMStyle, LanguageModel


def build_runner(args, model: LanguageModel, gen_num):
    if model.model_style == LMStyle.DoubaoAPI:
        from evaluate.runner.doubao_runner import DoubaoRunner
        return DoubaoRunner(args, model)
    elif model.model_style == LMStyle.DeepSeekAPI:
        from evaluate.runner.deepseek_runner import DeepSeekRunner
        return DeepSeekRunner(args, model, gen_num)
    elif model.model_style == LMStyle.LubanAPI:
        from evaluate.runner.luban_runner import LubanRunner
        return LubanRunner(args, model, gen_num)
    else:
        from evaluate.runner.vllm_runner import VLLMRunner

        return VLLMRunner(args, model)
    # if model.model_style == LMStyle.OpenAIChat:
    #     from evaluate.runner.oai_runner import OpenAIRunner
    #
