...
def compute_score(data_source, solution_str, ground_truth, extra_info=None) -> float:
    ...
    correctness_score = 1.0 if is_pass else 0.0
    in_outs = json.loads(extra_info.get("input_output"))
    cppcheck_error_count = get_valgrind_errors(solution_str, in_outs["inputs"])
    # logger.info(f"[valgrind_error_count]:\n {cppcheck_error_count}")
    alpha = 0.1
    penalty = alpha * cppcheck_error_count
    #
    reward = correctness_score - penalty
    reward = max(reward, 0.0)

    return cppcheck_error_count
